...
async function executeLocal(code, { onOutput, onError, onStatus }, language) {
    ...
    if (language.startsWith('python')) {
        ...
        const actualClassName = classMatch ? classMatch[1] : 'Main';
        const actualSourceFile = path.join(workDir, `${actualClassName}.java`);

        await fs.writeFile(actualSourceFile, cleanedCode, 'utf8');

        onStatus?.('Compiling...');

        const compileOk = await new Promise((resolve) => {
            execChild(
                `javac -cp "${JAVA_CLASSPATH}" "${actualSourceFile}"`,
        ...
        const proc = spawn('java', ['-cp', `${workDir}:${JAVA_CLASSPATH}`, actualClassName], {
            cwd: workDir,
            env: {
                ...process.env,
